...
- 对前面 *i - 1*个物品进行选择后，此时背包的剩余容量不够装下第 *i* 个物品了（不选择第 *i* 个物品）。

  也就是在这种选择方案下，*i* 个物品和 *i - 1* 个物品的最大价值是相同的。

- 剩余容量足够装下第 *i* 个物品，那么它需要占用 *weights[i]* 的容量。

  在这个选择方案中，其余的 *i - 1* 个物品收纳在 *j - weights[i]* 容量内（并不一定完全占满）。
